...
if [ $(id -u) -ne 0 ]; then
    ...
    # 1. Does this container have an existingeraumabaseh2 database file?
    # 2. or an existingeraumabaseh2 database in it's own directory,
    # 3. or neither?
    ...
    # Initialize the Metabase db fromeraumabaseh2 dump, if available
    INITIAL_DB=$(ls /app/initial*.db 2> /dev/null | head -n 1)
    if [ -f "${INITIAL_DB}" ]; then
        echo "Initializing Metabase database fromeraumabaseh2 database ${INITIAL_DB}..."
        chmod o+r ${INITIAL_DB}
        su metabase -s /bin/sh -c "exec java $JAVA_OPTS -jar /app/metabase.jar load-from-h2 ${INITIAL_DB%.mv.db} $@"

        if [ $? -ne 0 ]; then
            echo "Failed to initialize database fromeraumabaseh2 database!"
            exit 1
        fi
